...
data<- read.csv("ALLL Data.csv")
data$Date<- as.Date(as.character(data$Date), format = '%d-%b-%y')
source('functions.R')


#### steps 1: manully clean the data (lag or diff)
data1<- data[,c(2,19,3,16,14)]
head(data1)

data1$Y <-(data1$Y2/lag(data1$Y2,1))-1
data1$l5X1<-lag(data1$X1_B,5)
data1$dl2X14<-diffn(data1$X14_B,2)
...
reg<- arimax(data1$Y, order=c(1,0,0), 
             ...
             fixed = c(NA, NA, NA, NA, NA, NA, 0, 0, 0, NA))
reg
plot(data1$Y, fitted(reg))

reg<- auto.arima(data1$dY, xreg= cbind(l4X6=data1$l4X6,dX15=data1$dX15),
                 allowmean = F, approximation = F, stationary = T,
                 ic='aicc')

reg<- arima(data1$dY, xreg= cbind(l4X6=data1$l4X6,dX15=data1$dX15),
            include.mean = F, order=c(0,0,0))
# save residual
resi<- reg$residuals
...
write.xlsx(resiw, 'residualsALLL.xlsx', sheetName = 'ALLL')
