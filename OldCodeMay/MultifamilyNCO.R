...
data1<- data[,c(2,22,4,8,17)]
data1<-data1[-1,]
head(data1)


data1$dY<-diffn(data1$Y33,1)
data1$l3dX2<-lag(diffn(data1$X2_B,1),3)
data1$l3X6<-lag(data1$X6_B,3)
data1$dX15<-diffn(data1$X15_B,1)

reg<- arimax(data1$dY, order=c(1,0,0), 
             xtransf = cbind(l3dX2=data1$l3dX2, l3X6=data1$l3X6,dX15=data1$dX15), 
             transfer = list(c(0,0),c(0,0),c(0,0)), include.mean = T)
reg



Againreg<- arima(data1$Y34, xreg= cbind(l5dX13=data1$l5dX13, dX14=data1$dX14 ),
            include.mean = T, order=c(0,0,0))
reg


reg<-lm(data1$Y34~data1$l5dX13 + data1$dX14 )
summary(reg)
...
write.xlsx(resiw, 'residualsMultifamilyNCO.xlsx', sheetName = 'MultifamilyNCO')
