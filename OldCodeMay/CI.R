...
data1<- data[,c(2,20,8,17)]
head(data1)

data1$dY <-diffn(data1$Y6,1)
data1$l4X6<-lag(data1$X6_B,4)

data1$lX15<-diffn(data1$X15_B,1)


reg<- arima(data1$dY, order=c(1,0,0), 
             xreg = cbind(l4X6=data1$l4X6, lX15=data1$lX15), 
              include.mean = F)
reg

#reg<-lm(data1$dY~data1$l4X6+data1$lX15+ 0)
#reg
resi<- reg$residuals
...
write.xlsx(resiw, 'residualsCIBalance.xlsx', sheetName = 'CIBalance')
