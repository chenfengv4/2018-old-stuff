...
data1$dY<-diffn(data1$Y21,1)
data1$ldX5<-lag(diffn(data1$X5_B,1),1)
data1$l4dX7<-lag(diffn(data1$X7_B,1),4)
reg<- lm(data1$dY~data1$ldX5  + data1$l4dX7+0)

summary(reg)

reg<-arima(data1$dY, order=c(0,0,0),xreg=cbind(data1$ldX5, data1$l4dX7),  include.mean = F)
reg
...
write.xlsx(resiw, 'residualMultifamilyNA.xlsx', sheetName = 'MultifamilyNA')
