...
data1<- data[,c(2,22,6,16,29)]
head(data1)

data1$lnY<- log(data1$Y54)
data1$dlnY<-diffn(data1$lnY,1)

data1$lX4<-lag(data1$X4_B,1)
data1$X14DD<- lag(data1$X14_B,1)-2*lag(data1$X14_B,2)+lag(data1$X14_B,3)

data1$dl5D5<-lag(diffn(data1$D_5,1),5)
data1$l5D5<- lag(data1$D_5,5)



reg<- arima(data1$dlnY, order=c(0,0,0), 
            xreg =  cbind(lX4=data1$lX4, X14DD=data1$X14DD, dl5D5=data1$dl5D5), 
            include.mean = F)
reg
...
write.xlsx(resiw, 'residualsRegularSavingAccount.xlsx', sheetName = 'RegularSavingAccount')
