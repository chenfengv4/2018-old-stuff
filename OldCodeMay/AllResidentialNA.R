...
data1$dY<-diffn(data1$Y24,1)
data1$l4X6<-lag(data1$X6_B, 4)
data1$l4dX14<-lag(diffn(data1$X14_B,1), 4)

reg<- lm(data1$dY~data1$l4X6  + data1$l4dX14+0)

summary(reg)

reg<-arima(data1$dY, order=c(0,0,0),xreg=cbind(data1$l4X6, data1$l4dX14),  include.mean = F)
reg
...
write.xlsx(resiw, 'residualAllResidentialNA.xlsx', sheetName = 'AllResidentialNA')
