...
data1<- data[,c(2,21,3,15)]
head(data1)
#data1$Y= NA
data1$dY <-diffn(data1$Y9,1)

data1$l6d1X13<-lag(diffn(data1$X13_B,1), 6)

#data1<-data1[8:64,]
reg<- arima(data1$dY, order=c(0,0,0), 
             xreg =   cbind(X1=data1$X1_B,l6d1X13=data1$l6d1X13), 
              include.mean = F)
reg<-lm(data1$dY~data1$X1_B + data1$l6d1X13 + 0)
reg
...
write.xlsx(resiw, 'residualsMultifamilyBalance.xlsx', sheetName = 'MultifamilyBalance')
