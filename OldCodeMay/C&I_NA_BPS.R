...
data1<- data[,c(2,19,7,4)]


head(data1)


data1$dY<-diffn(data1$Y18,1)

data1$l4dX5<-lag(diffn(data1$X5_B,1),4)
data1$ldX2<-lag(diffn(data1$X2_B,1),1)
reg<- lm(data1$dY~data1$l4dX5  + data1$ldX2+0)

summary(reg)

reg<-arima(data1$dY, order=c(0,0,0),xreg=cbind(data1$l4dX5, data1$ldX2),  include.mean = F)
reg
...
write.xlsx(resiw, 'residualC&I_NA.xlsx', sheetName = 'C&I_NA')
