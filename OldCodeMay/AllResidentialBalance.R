...
data1<- data[,c(2,22,3,8,16)]
head(data1)
#data1$Y= NA
data1$lnY <-log(data1$Y12)
data1$dlnY<-diffn(data1$lnY,1)
data1$l3X1<-lag(data1$X1_B,3)
data1$l3X6<-lag(data1$X6_B,3)
data1$lX14<-lag(data1$X14_B,1)
#data1<-data1[8:64,]
reg<- arimax(data1$dlnY, order=c(0,0,0), 
             xtransf = cbind(l3X1=data1$l3X1,l3X6=data1$l3X6, lX14=data1$lX14), 
             transfer = list(c(0,0),c(0,0),c(0,0)), include.mean = F)

reg<- arima(data1$dlnY, order=c(0,0,0), 
             xreg = cbind(l3X1=data1$l3X1,l3X6=data1$l3X6, lX14=data1$lX14), 
              include.mean = F)

reg<-lm(data1$dlnY~data1$l3X1+ data1$l3X6 + data1$lX14 + 0)
reg
...
write.xlsx(resiw, 'residualsAllResidentialBalance.xlsx', sheetName = 'AllResidentialBalance')
