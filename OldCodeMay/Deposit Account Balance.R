...
data1<- data[,c(2,19,3,14)]
head(data1)

data1$lnY <- log(data1$Y51)
data1$dlnY <- diffn(data1$lnY,1)

data1$lX1<- lag(data1$X1_B,1)
data1$lnX12<-log(data1$X12_B)
data1$dlnX12<-diffn(data1$lnX12,1)

data1$llnX12<-lag(data1$dlnX12,1)



reg<- arima(data1$dlnY, order=c(0,0,0), 
            xreg =  cbind(X1_B=data1$X1_B,ldiffX12_B= data1$llnX12), 
            include.mean = F)
reg
...
write.xlsx(resiw, 'residualsNIBDDA.xlsx', sheetName = 'NoninterestBearingDemandDepositAccounts')
